...
Pure algebra from the config -- no simulation. Covers:
  ...
  - receiver noise floor (shot, thermal, dark, quantization)
  - NEP referred to optical input
  - dynamic range

Geometric quantities (dz, f_beat, Nyquist headroom) live in
core.config.compute_derived -- this module is the power/noise side.

See issue #43. Strain/temperature sensitivity and full phase-noise
contribution are intentionally left for a later pass.
"""
...
def compute_budget(cfg: dict[str, Any]) -> dict[str, float]:
    ...
    sigma_total = math.sqrt(
        sigma_shot ** 2 + sigma_thermal ** 2 + sigma_dark ** 2 + sigma_quant ** 2
    )
